...
class Workspace extends Component {
    ...
    constructor(params) {
        ...
        self.terminal = null;
        this.current = {};
        this.context = { "context" : "env", "status" : "pending", "output" : [], "uid" : Config.env, "env" : Config.env, "project" :  Config.project, "env_name" : Config.name_env,  "project_name" : Config.name_project, "workdir" : "" };
        this.action = "";
    ...
    applyAction(evt, self) {
        ...
        if (action === "run-env") {
            self.runEnvActionApply(action, uid);
        }
        ...
        if( action === "run-env" || action == "run-container" || action === "open-terminal" ) {
           /*
    ...
    removeActionApply(element, uid) {
        ...
        self.http.url = url;
        self.http.fetch(function(response) {
           self.displayMessage(response.status, response);
        });
    }
    ...
    displayMessage(status, message) {
        this.renderOtherComponent("Message", { status : status, "message" :  message }, dom.get("body"), null);
    }
    ...
    renderOutputEvents(data) {
        ...
        var element = dom.get(".widget-parent-event[data-slug*='"+data.output.name+"']");
        if ( element !== null ) {
...
//invock.mount({ parent : "body", root : "{% Workspace %}" });