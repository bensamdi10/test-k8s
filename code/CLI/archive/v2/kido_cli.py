...
class KidoCLI:
    ...
    async def runCommand(self, command , websocket, data_init, finish="true"):
        ...
        if processus.returncode != 0:
            ...
            data_send_termine = self.sendOutput(message_error, data_init["uid"], "output-terminal", [], "", "true")
            await websocket.send(data_send_termine)

            return False

        data_send_termine = self.sendOutput("", data_init["uid"], "output-terminal", [], "", finish)
        await websocket.send(data_send_termine)
    ...
    async def runTaskCommands(self, list_commands, websocket, data_init, message_termine="", finish="true"):
        pass_all = True
        for command in list_commands:
            result = await self.runCommand(command, websocket, data_init, finish)
            if result == False:
                pass_all = False
                break
        if pass_all == True:
            data_send_termine = self.sendOutput(message_termine, data_init["uid"], "output-terminal", [], "", "true")
            await websocket.send(data_send_termine)
            ...
            async with websockets.connect(uri) as websocket:
                while True:
                    ...
                    if action == "run-env":
                        ...
                        data_send_start = self.sendOutput("Generate Files Config for Kubernetes Cluster...", uid, "output-terminal", [], "false")
                        ...
                        await self.runTaskCommands(commands_start_minikube, websocket, recev_data, "Minikube Started", "true")
    ...
    def runCommandSync(self, command, data_init, finish):
        processus = subprocess.Popen([command], shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        output, error = processus.communicate()
        data_send_termine = ""
        if processus.returncode != 0:
    ...
    async def runTaskCommandsSync(self, list_commands, websocket, data_init, finish="true", message=""):
        for command in list_commands:
            click.echo(command)
        ...
        data_send_message = self.sendOutput(message, data_init["uid"], "output-terminal", [], "", "true")
    ...
    async def runContainer(self, uid, websocket, data_init, extra_data):
        ...
        await self.runTaskCommandsSync(commands_container, websocket, data_init, "false", "Image of COntainer "+data_init['extra_data']['name'])
    ...
    def initCLI(self):
        ...
        processus = subprocess.Popen(["docker run -it said10/plateforme"], shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    ...
    def sendOutput(self, output, uid, action, errors, workdir="", finish_output="true", extra_data={}):
        data = {
            "action": action,
            "uid": uid,
            "output": output,
